...
void * crash_return(void * args) {
	...
	pthread_mutex_lock(&(crash_mutex));
	crash_now = TRUE; //was removed this for testing
	pthread_mutex_unlock(&(crash_mutex));

	return NULL;
}

/*used to force crashes at specific points in code for more precise testing
I removed the other crash thread and used this to test by forcing it to crash at points of interest
*/
void crashNow() {
	fprintf(stderr, "Crashing now\n");
	pthread_mutex_lock(&(crash_mutex));
	crash_now = TRUE; 
	pthread_mutex_unlock(&(crash_mutex));
}
